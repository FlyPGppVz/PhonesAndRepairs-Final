  ...
  return (
    <div className="fixed bottom-[20px] right-[20px] z-[9999] flex flex-col items-end gap-[10px] pointer-events-none scale-85 md:scale-100 transition-transform duration-300 ease-in-out">
      ...
      <div className="flex flex-col items-end gap-4 pointer-events-auto">
        ...
        >
          <div className="relative bg-white dark:bg-neutral-800 text-blue-600 dark:text-blue-400 h-14 pr-4 pl-4 rounded-full shadow-2xl border border-slate-100 dark:border-white/5 hover:scale-110 active:scale-95 transition-all duration-300 flex items-center justify-center gap-2">
            <div className="absolute inset-0 bg-blue-500/10 rounded-full opacity-0 group-hover:opacity-100 group-hover:animate-ping pointer-events-none"></div>
            <span className="max-w-0 overflow-hidden group-hover:max-w-[150px] transition-all duration-500 ease-out font-black text-[10px] uppercase tracking-[0.2em] whitespace-nowrap opacity-0 group-hover:opacity-100">
              Repair Services
            </span>
        ...
        <button
          ...
          className={`group relative animate-fade-in-up ${isChatOpen ? 'rotate-90' : ''}`}
          style={{ animationDelay: '0.6s' }}
          aria-label={isChatOpen ? "Close Chat" : "Open Chat"}
        >
          <div className={`relative w-14 h-14 flex-shrink-0 rounded-full shadow-2xl transition-all duration-300 flex items-center justify-center ${isChatOpen ? 'bg-red-500 text-white' : 'bg-blue-600 text-white hover:shadow-blue-500/40 hover:scale-110 active:scale-95'}`}>
            {!isChatOpen && <div className="absolute inset-0 bg-white/20 rounded-full opacity-0 group-hover:opacity-100 group-hover:animate-ping pointer-events-none"></div>}
            <span className="material-symbols-outlined text-[28px]">{isChatOpen ? 'close' : 'chat'}</span>
          </div>
        </button>
      </div>
    </div>
